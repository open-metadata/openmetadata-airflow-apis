...
from metadata.generated.schema.api.workflows.operations.workflow import WorkflowConfig
...
dag_managed_operators = configuration.get(
    "openmetadata_airflow_apis", "DAG_MANAGED_OPERATORS"
)
        ...
        "post_arguments": [
            {
                ...
                "description": "Workflow config to deplooy",
...
class REST_API(get_baseview()):
    ...
    def get_dagbag():
        dagbag = DagBag(
            dag_folder=settings.DAGS_FOLDER, read_dags_from_db=store_serialized_dags
        )
        dagbag.collect_dags()
    ...
    def delete_dag(self):
        ...
        try:
            ...
            error_message = (
                "An error occurred while trying to delete the DAG '"
                + str(dag_id)
                + "': "
                + str(e)
            )
    ...
    def list_run(self):
        ...
        try:
            ...
            dag_runs = query.filter(DagRun.dag_id == dag_id).all()
            ...
            return ApiResponse.server_error(
                "Failed to list dag runs for {}".format(dag_id)
            )
    ...
    def restart_failed_task(self):
        ...
        return ApiResponse.success(
            {"failed_task_count": failed_task_count, "clear_task_count": count}
        )
